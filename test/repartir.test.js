...
describe('Repartir', () => {
  ...
  debera finalizar en la posicion x:-2 y:4 orientacion:[O]este`, async () => {
    const dron = new Dron([['A', 'A', 'A', 'A', 'I', 'A', 'A']], 'in01.txt');
    const { entregas } = await repartir(dron);
    expect(entregas).to.be.an('array').and.is.not.empty;
    expect(entregas[0])
      .to.be.an('object')
  ...
  debera finalizar en la posicion x:-2 y:4 orientacion:[O]este`, async () => {
    const dron = new Dron([['D', 'D', 'D', 'A', 'I', 'A', 'D']], 'in01.txt');
    const { entregas } = await repartir(dron);
  ...
  debera finalizar en la posicion x:-2 y:4 orientacion:[O]este`, async () => {
    const dron = new Dron([['A', 'A', 'I', 'A', 'D', 'A', 'D']], 'in01.txt');
    const { entregas } = await repartir(dron);
